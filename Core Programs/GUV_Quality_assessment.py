...
def ratio_extractor (direct):
   ...
   for n in range(len(df_filenames)):
     ...
     for key in store.keys():
        ...
        '''
        if df ['radius_micron'].tolist()[0] > 4:
          df_pos += 1
        '''
        df_total +=1 
        df_list.append(df)
   ...
   ratio_list = np.array(df_total_list) / (1024*1024*0.325*0.325)
...
plt.xlabel('')
plt.ylabel('unilammellar/multilammellar', fontsize=14, fontweight = 'bold')
