...
def curve_df_combine(directory):
   ...
   for n in range(len(df_filenames)):
      ...
      for key in store.keys():
         ...
         #df['GFP intensity'] = df['GFP intensity'] /Normalize_factor
         df_list.append(df)
...
# Import first dataset for normalization
#root.directory = filedialog.askdirectory()

#df_mean = Extract_df_mean(root.directory)

df_final_one,df_final_one_len = curve_df_combine(Name1)
#print(df_final_one)

# Import second dataset for normalization
#root.directory = filedialog.askdirectory()
...
df_final_three,df_final_three_len = curve_df_combine(Name3,df_mean)
...
df_final_four,df_final_four_len = curve_df_combine(Name4,df_mean)


root.directory = filedialog.askdirectory()
Name5 = root.directory
label_5 = Name5.split("/")[-1]
df_final_five,df_final_five_len = curve_df_combine(Name5,df_mean)
...
sns.lineplot(x='Time Point', y='Normalized GFP intensity', data = df_final_one,label='%s (n = %d)' %(label_1, df_final_one_len), color='r')

ax.tick_params(axis = 'y', labelcolor = 'r', labelsize = 'x-large')
ax.tick_params(axis = 'x',  labelsize = 'x-large')
ax.set_xlabel('Time Points (min)', fontweight = 'bold', fontsize = 20)
ax.set_ylabel('Fluorescence Intensities', fontweight = 'bold', fontsize = 20, color = 'r')
...
sns.lineplot(x='Time Point', y='Normalized GFP intensity', data = df_final_two,label='%s (n = %d)' %(label_2, df_final_two_len), ax = ax2,color='b')
...
ax2.set_ylabel('Fluorescence Intensities', fontweight = 'bold', fontsize = 20, color = 'b')
...
#ax.tick_params(axis = 'x',  labelsize = 'x-large')
#ax.set_xlabel('Time Points (min)', fontweight = 'bold', fontsize = 20)
#ax.set_ylabel('Alox12-EGFP Normalize Fluorescence Intensities', fontweight = 'bold', fontsize = 16)
...
ax2.legend(lines + lines2, labels + labels2, loc='center left', bbox_to_anchor=(0.4, 0.9))
